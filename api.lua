...
function bls_mobs.perpendicular_vector(vec) --returns a vector rotated of 90° in 2D
	local ang = math.pi/2
...
function bls_mobs.add_entity_and_particles(entity, pos, particles, multiplier)
...
function bls_mobs.node_ok(pos, fallback)
...
function bls_mobs.dist_pos(p, s)
...
function bls_mobs.check_for_death_hydra(self)
...
function bls_mobs.explosion_particles(pos, exp_radius)
    minetest.add_particlespawner(
...
function bls_mobs.digging_attack(
	...
	if self.attack and self.attack:is_player() then
		...
		local per = bls_mobs.perpendicular_vector(dir)
...
function bls_mobs.putting_ability(		--puts under the mob the block defined as 'p_block'
	self,		--the entity of the mob
	...
	if n~=p_block and not minetest.is_protected(pos, "") and (n == "bones:bones" and bls_mobs:affectbones(self) ) and n~="air" then
		minetest.env:set_node(pos, {name=p_block})
		if bls_mobs.cryosave then
			local metai = minetest.get_meta(pos)
			metai:from_table(oldmetainf[1]) -- this is enough to save the meta
			metai:set_string("bls_mobs",n)
		end
	end
	if n1~=p_block and not minetest.is_protected(pos1, "") and (n == "bones:bones" and bls_mobs:affectbones(self) ) and n~="air" then
		...
		if bls_mobs.cryosave then
			...
			metai:set_string("bls_mobs",n1)
		end
	end
end


function bls_mobs.webber_ability(		--puts randomly around the block defined as w_block
	self,		--the entity of the mob
	w_block, 	--definition of the block to use
	radius		--max distance the block can be put
	)

	if (bls_mobs:virulence(self)~=0) and (math.random(1,bls_mobs:virulence(self)) ~= 1) then return end
...
function bls_mobs.midas_ability(		--ability to transform every blocks it touches in the m_block block
	self,		--the entity of the mob
	m_block,
	max_vel,	--max velocity of the mob
	...
	if x>0 then
		i = bls_mobs.round(x*max_vel)*multiplier
	else
		i1 = bls_mobs.round(x*max_vel)*multiplier
	end

	if z>0 then
		k = bls_mobs.round(z*max_vel)*multiplier
	else
		k1 = bls_mobs.round(z*max_vel)*multiplier
	end
	...
	for _, obj in pairs(objs) do
		...
		if obj:is_player() then
			...
			if objdef and objdef.on_blast then
				if ((name == "bls_mobs:pumpking") or (name == "bls_mobs:morvalar0") or (name== "bls_mobs:morvalar5")) then
					do_damage = false
					do_knockback = false
				else
...
function bls_mobs.tnt_boom_bls_mobs(pos, def)
